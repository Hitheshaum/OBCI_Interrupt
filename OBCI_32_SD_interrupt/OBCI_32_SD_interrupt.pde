 ...
 * Modified to use interrupts by Hithesh Reddivari, 2015.
 *
 ...
 * Use at your own risk, wysiwyr.

 * Points to be noted:
 * Dont put serial outs when data transfer. 
 * Clear the interrupt flag immediately after entering ISR and immediately before enabling interrupt.
 * PPSunlock sequence for some reason disables the data transfer.
 * 
 */
...
void setup(void) {
  ...
  IEC0bits.INT4IE=0; // Disable INT4
  INTCONbits.INT4EP=0; //clear bit for falling edge trigger (ADS_DRDY pin goes low when data is available)
  IPC4bits.INT4IP=7;// interrupt4 priority. If you change this make sure you also change at the ISR
  IPC4bits.INT4IS=1;//interrupt4 subpriority
  IFS0bits.INT4IF=0; //clear interrupt flag

  //PPS unlock and lock sequences are necessary for assigning an interrrupt to a pin
  //This block for unknown reasons disables OpenBCI data transfer
  //Fortunately we don't have to change pin as interrupt4 defaults to the pin we want
  // PPSUnlock;
...
void loop() {
  ...
  // The loop is all yours. Just make sure you somehow synchronize with the data from ISR
...
void eventSerial(){
  while(Serial0.available()){      
    ...
    if(plusCounter == 1){  // if we have received the first 'bun' (first +)
      testChar = inChar;   // this might be the command
      plusCounter++;       // get ready to look for another 'bun' (next +)
